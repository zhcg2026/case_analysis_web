...
# MIGRATED: /api/assess/v2 已迁移到 assessment_routes.py
# @app.route('/api/assess/v2', methods=['POST'])
# @protected
def _migrated_assess_v2():
    pass

# MIGRATED: /api/spotcheck 已迁移到 assessment_routes.py
# @app.route('/api/spotcheck', methods=['POST'])
# @protected
def _migrated_spotcheck():
    pass
...
# @app.route('/api/categories', methods=['GET'])  # 已迁移到 cms_routes.py
# @protected
def _unused_get_categories():
    # 创建新的session实例
    session = Session()
    try:
        # 获取所有栏目，按排序字段排序
...
# 小工具模块API - 自然语言查询转换为SQL  # 已迁移到 analysis_routes.py
# @app.route('/api/tools/natural-language-query', methods=['POST'])
# @protected
def _unused_natural_language_query():
    try:
        data = request.json
...
# @protected
def _unused_get_desensitization_fields():
    """获取Excel文件的所有字段名"""
    try:
...
# 小工具模块API - 数据脱敏处理  # 已迁移到 analysis_routes.py
# @app.route('/api/tools/data-desensitization', methods=['POST'])
# @protected
def _unused_process_desensitization():
    """处理数据脱敏并返回处理后的文件"""
    try:
        if 'file' not in request.files:
            return jsonify({'error': '没有文件'}), 400
...
# @app.route('/api/categories', methods=['POST'])  # 已迁移到 cms_routes.py
# @admin_required
def _unused_create_category():
    session = Session()
    try:
        data = request.json
...
# @app.route('/api/categories/<int:category_id>', methods=['GET'])  # 已迁移到 cms_routes.py
# @protected
def _unused_get_category(category_id):
    session = Session()
    try:
...
# @app.route('/api/categories/<int:category_id>', methods=['PUT'])  # 已迁移到 cms_routes.py
# @admin_required
def _unused_update_category(category_id):
    session = Session()
    try:
        category = session.query(Category).filter_by(id=category_id).first()
        if not category:
...
# @app.route('/api/categories/<int:category_id>', methods=['DELETE'])  # 已迁移到 cms_routes.py
# @admin_required
def _unused_delete_category(category_id):
    session = Session()
    try:
...
# @app.route('/api/data-edit/records', methods=['GET'])  # 已迁移到 data_edit_routes.py
# @admin_required
def _unused_get_data_edit_records():
    """获取数据列表（支持分页、筛选、查找）"""
    try:
...
# DUPLICATE: @app.route('/api/data-edit/record', methods=['POST'])
# @admin_required
def _dup_create_data_edit_record():
    """新增记录"""
    session = Session()
    try:
...
# DUPLICATE: @app.route('/api/data-edit/record/<task_number>', methods=['PUT'])
# @admin_required
def _dup_update_data_edit_record(task_number):
    """修改记录"""
    session = Session()
    try:
        data = request.get_json()
...
# DUPLICATE: @app.route('/api/data-edit/record/<task_number>', methods=['DELETE'])
# @admin_required
def _dup_delete_data_edit_record(task_number):
    """删除单条记录"""
    session = Session()
    try:
        table_name = request.args.get('table_name')
...
# DUPLICATE: @app.route('/api/data-edit/batch-delete', methods=['POST'])
# @admin_required
def _dup_batch_delete_data_edit_records():
    """批量删除记录"""
    session = Session()
    try:
        data = request.get_json()
...
# DUPLICATE: @app.route('/api/data-edit/batch-update', methods=['POST'])
# @admin_required
def _dup_batch_update_data_edit_records():
    """批量修改记录"""
    session = Session()
    try:
        data = request.get_json()
...
# @app.route('/api/operation-logs', methods=['GET'])  # 已迁移到 data_edit_routes.py
# @admin_required
def _unused_get_operation_logs():
    """获取操作日志列表"""
    try:
        table_name = request.args.get('table_name', '')
...
# ==================== 知识库 RAG API ====================  # 已迁移到 knowledge_routes.py

# @app.route('/api/knowledge/stats', methods=['GET'])
# @protected
def _unused_knowledge_stats():
    """获取知识库统计信息"""
    try:
...
# DUPLICATE: @app.route('/api/knowledge/documents', methods=['GET'])
# @protected
def _dup_knowledge_list_documents():
    """列出知识库中的所有文档"""
    try:
        docs = list_documents()
...
# DUPLICATE: @app.route('/api/knowledge/documents/<doc_id>', methods=['DELETE'])
# @protected
def _dup_knowledge_delete_document(doc_id):
    """删除指定文档"""
    try:
...
# DUPLICATE: @app.route('/api/knowledge/documents/batch-delete', methods=['POST'])
# @protected
def _dup_knowledge_batch_delete():
    """批量删除文档"""
    try:
        data = request.get_json()
...
# DUPLICATE: @app.route('/api/knowledge/documents/delete-all', methods=['POST'])
# @protected
def _dup_knowledge_delete_all():
    """删除所有文档"""
    try:
...
# DUPLICATE: @app.route('/api/knowledge/upload', methods=['POST'])
# @protected
def _dup_knowledge_upload_document():
    """上传文档到知识库"""
    try:
        # 检查是否有文件
        if 'file' not in request.files:
...
# DUPLICATE: @app.route('/api/knowledge/batch-upload', methods=['POST'])
# @protected
def _dup_knowledge_batch_upload():
...
# DUPLICATE: @app.route('/api/knowledge/batch-upload/progress/<task_id>', methods=['GET'])
# @protected
...
# DUPLICATE: @app.route('/api/knowledge/search', methods=['POST'])
# @protected
def _dup_knowledge_search():
    """搜索知识库"""
    try:
        data = request.get_json()
...
# DUPLICATE: @app.route('/api/knowledge/ask', methods=['POST'])
# @protected
def _dup_knowledge_ask():
...
# DUPLICATE: @app.route('/api/knowledge/init', methods=['POST'])
# @admin_required
def _dup_knowledge_init():
    """初始化RAG模块（管理员权限）"""
    try:
        init_rag()
...
# @protected
def _migrated_check_jurisdiction():
    pass
