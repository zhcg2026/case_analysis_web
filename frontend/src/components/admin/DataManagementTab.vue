    ...
    <div class="content-card">
      ...
      <div class="card-body">
        ...
        <div class="action-bar" v-if="selectedIds.length > 0">
          ...
          <button class="btn btn-secondary" @click="showBatchEditModal = true">批量修改</button>
...
const searchableColumns = computed(() =>
  columns.value.filter(c => c.editable && ['varchar', 'text'].includes(c.type))
)
...
/* 批量操作栏 */
.action-bar { display: flex; align-items: center; gap: 12px; padding: 8px 12px; background: var(--primary-50, rgba(59,130,246,0.05)); border-radius: 6px; margin-bottom: 12px; font-size: 13px; }
